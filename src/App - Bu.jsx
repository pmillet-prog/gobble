// Fichier UTF-8 : conserver les accents, emojis et rÃ¨gles de normalisation (Å, etc.). Ne pas convertir d'encodage.
// 
...
// Hauteur cible du bloc principal : clamp sur la fenÃªtre pour éviter les colonnes infinies en zoom/dézoom
...
const BASE_GAP_PX = 8; // gap-2 de réfÃ©rence
const BASE_GAP_RATIO = BASE_GAP_PX / BASE_TILE_PX; // ~0.14 pour conserver les proportions
const MIN_TILE_SIZE = 40; // garde une lisibilitÃ© minimale
...
body.theme-dark .ring-offset-white {
  --tw-ring-offset-color: #0d1625 !important; /* fond derriÃ¨re l'anneau */
}
...
.big-score-burst {
  ...
  /* mÃªme palette que les tuiles de base : bg-orange-200 / border-orange-500 */
  ...
  font-size: 1.1rem;
  box-shadow:
    -1px 0 0 rgba(15, 23, 42, 0.20),   /* ombre cÃ´tÃ© gauche */
    0 2px 0 rgba(15, 23, 42, 0.28),    /* ombre en bas */
    0 6px 12px rgba(15, 23, 42, 0.45); /* ombre portÃ©e globale */
...
body.theme-dark .preview-tile {
  ...
  position: relative;
  box-shadow:
    -1px 0 0 rgba(15, 23, 42, 0.25),   /* ombre cÃ´tÃ© gauche */
    0 2px 0 rgba(15, 23, 42, 0.32),    /* ombre en bas */
    0 6px 12px rgba(15, 23, 42, 0.35); /* ombre portÃ©e globale */
  transform-origin: center center;
  overflow: hidden;
}
...
body.theme-dark .board-tile::after {
  /* un peu plus doux en dark pour Ã©viter la bouillie */
  background:
...
export default function App() {
  ...
  // DÃ©bloque le contexte audio au premier geste utilisateur (mobile/desktop)
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    // init immÃ©diat (on enlÃ¨ve un petit padding interne pour coller au contenu)
 ...
 useEffect(() => {
  ...
  // Tant que le mode 5x5 n'est PAS dÃ©bloquÃ©, on force le 4x4
  ...
  // Son "GOBBLE" (MP3 placÃ© dans /public/GOBBLE.mp3)
  function playGobbleVoice() {
    ...
    // Petit "bip" progressif Ã  chaque tuile ajoutÃ©e
function playTileStepSound(step) {
  ...
  const start = () => {
    ...
    // A4 comme base, gamme pentatonique (Ã§a reste consonant)
    const baseFreq = 440;
  ...
  const start = () => {
    ...
    // 1 1 3 5 3 5 en degrÃ©s de gamme majeure -> 0,0,4,7,4,7 demi-tons
    const semitones = [0, 0, 4, 7, 4, 7];

    // DurÃ©e et tempo de la petite phrase
    const noteDur = 0.14;      // durÃ©e de chaque note
  ...
  // Synchronise l'Ã©tat muet avec l'Ã©lÃ©ment audio (certaines plateformes le gardent en mÃ©moire)
  useEffect(() => {
  ...
  // Palette de sons par paliers de score (plus mÃ©lodique)
  ...
  function playScoreSound(points) {
    ...
    const start = () => {
      ...
      band.intervals.forEach((semi, idx) => {
        ...
        try {
          osc.start(now + idx * 0.01); // trÃ¨s lÃ©ger dÃ©calage pour l'effet arpÃ¨ge
  ...
  function showToast(message) {
    ...
    // Si plus d'1,2s se sont Ã©coulÃ©es, on repart de zÃ©ro
    if (now - lastTs > 1200) {
    ...
    if (newCount >= 3) {
      // Triple tap : on bascule le mode 5x5 mÃªme sur mobile
      secretTapRef.current = { count: 0, lastTs: now };
      setBigGridUnlocked((prev) => {
        const next = !prev;
        try {
          showToast(
            next ? "Mode 5x5 dÃ©bloquÃ© sur mobile" : "Retour au mode standard"
          );
        } catch (_) {}
        return next;
      });
  ...
  function getTileIndexFromPoint(x, y) {
    for (let i = 0; i < tileRefs.current.length; i++) {
      ...
      const radius = Math.min(rect.width, rect.height) * 0.38; // zone circulaire rÃ©duite
  ...
  useEffect(() => {
   ...
   useEffect(() => {
    ...
    if (phase === "playing") {
      // Si le serveur fournit une heure de fin, on l'utilise comme source de vÃ©ritÃ©
      if (serverEndsAt) {
        id = setInterval(() => {
          setTick(() => {
            const now = Date.now();
            ...
            if (remaining <= 0) {
              ...
              if (dictionary) {
                ...
                console.log("RÃ©sultats trouvÃ©s :", all);
                setAllWords(all);
              }
  ...
  // Bots dÃ©sactivÃ©s : on ne complÃ¨te plus les rÃ©sultats localement
  useEffect(() => {
    /* no-op */
  }, [phase, allWords, roundId, nickname]);

  // Attribue des mÃ©dailles locales Ã  la fin d'une manche
  useEffect(() => {
  ...
  /**
   * Ajout de lettres via le clavier, avec pathfinder optimisÃ©.
   */
  ...
  useEffect(() => {
    function onKey(e) {
      ...
      // On ne gÃ¨re le reste que si le jeu est la zone active
...
setCurrentTiles((prevLetters) => {
  ...
  const step = newLetters.length - 1; // 0 pour la premiÃ¨re, 1 pour la 2e, etc.
...
function handleTouchEnd() {
  ...
  // surtout pas de preventDefault ici, Ã§a peut foutre le bazar sur mobile
  function handleMouseUp() {
  if (!draggingRef.current) return;
  draggingRef.current = false;
  submit();
  ...
  if (typeof window !== "undefined") {
    ...
    // Mode en ligne : on dÃ©lÃ¨gue la validation / le score au serveur
    if (roundId && socket.connected && isLoggedIn) {
      socket.emit("submitWord", { roundId, word: raw }, (res) => {
        if (!res || !res.ok) {
          if (res?.error === "invalid_word") {
            return error("Mot refusÃ© par le serveur");
          }
          if (res?.error === "already_played") {
            return error("Mot dÃ©jÃ  jouÃ© (serveur)");
          }
...
showToast(`+${pts} pts`);
  ...
  function submitChat(e, forcedText = null) {
    ...
    socket.emit("chat:send", text, (res) => {
      if (!res?.ok) {
        setConnectionError("Message non envoyÃ©");
      } else {
        setConnectionError("");
      }
    });
  ...
  const currentDisplay =
    currentTiles.length > 0
      ...
      ? statusMessage
      : "";
        // Mot en cours d'Ã©criture : on prend l'Ã©tat, et si jamais
  ...
  // messages visibles dans le chat (max 18), ancrÃ©s en bas
  ...
  function buildTestBots(all) {
    ...
    for (let i = 0; i < BOT_COUNT; i++) {
      const level = 0.05 + Math.random() * 0.35; // 5% Ã  40% du dico
  ...
  function playBotWord(nick) {
    ...
    // Ã©vite de rejouer les mots de l'humain pour rendre le classement plus lisible
  ...
  const gapRatio = Math.max(0.08, Math.min(0.18, BASE_GAP_RATIO * (4 / gridSize))); // 4x4 inchangÃ©, 5x5 resserrÃ©
    ...
    Salon multijoueur sans compte...
            ...
            <div className="flex flex-col items-start gap-1 text-xs">
              <span className="px-3 py-1 rounded-full bg-white/10 border border-white/10">
                {isConnecting ? "Connexion..." : "Serveur en Ã©coute"}
              </span>
              {connectionError && (
          ...
          <div className="grid md:grid-cols-3 gap-4 text-sm text-slate-200">
            <div className="p-3 rounded-lg bg-white/5 border border-white/10">
              ...
              <div className="text-slate-300 text-xs">
                Pas de compte, juste un pseudo non utilisÃ© par un autre joueur.
              </div>
            </div>
            <div className="p-3 rounded-lg bg-white/5 border border-white/10">
              <div className="font-semibold">Chat en direct</div>
              <div className="text-slate-300 text-xs">
                Messages partagÃ©s dÃ¨s que tu es connectÃ©.
              </div>
            </div>
            <div className="p-3 rounded-lg bg-white/5 border border-white/10">
              <div className="font-semibold">Classement live</div>
              <div className="text-slate-300 text-xs">
                Ta position se met Ã  jour, les scores dÃ©taillÃ©s arrivent en fin de manche.
              </div>
            </div>
          </div>
  ...
  // === Mise en page mobile dÃ©diÃ©e pendant la manche ===
  // Ãcran unique : classement + prÃ©visualisation du mot + grille en bas + bouton de chat
  if (isMobileLayout && (phase === "playing" || phase === "results")) {
    const isResults = phase === "results";
    const fullRanking = isResults ? finalRanking || [] : rankingSource || [];
    const mobileAnnouncements = mixedFeed.slice(0, 8);
            ...
            <div className="flex items-center justify-between gap-2">
              ...
              <div className="flex items-center gap-2">
                ...
                >
                  ...
                  <span className="sr-only">{isMuted ? "Son coupÃ©" : "Son actif"}</span>
                </button>
                <button
  ...
  >
        ...
        {/* En-tÃªte compact : titre, salon, score et boutons rapides */}
        <div className="px-3 pt-2 pb-1 border-b border-slate-200/70 dark:border-slate-700/70">
          <div className="flex items-center justify-between gap-2">
            <div className="flex flex-col">
              <div className="text-lg font-extrabold tracking-tight leading-none">GOBBLE</div>
...
>
  ...
  <span className="sr-only">
    {isFullscreen ? "Quitter le plein Ã©cran" : "Passer en plein Ã©cran"}
  </span>
</button>
        ...
        {showHelp && (
          <div className="mx-3 mt-2 mb-1 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-xl p-3 text-[11px] text-gray-700 dark:text-slate-200">
            ...
            <ul className="list-disc list-inside space-y-1">
              ...
              <li>EntrÃ©e valide le mot, Backspace efface.</li>
              ...
              <li>Score = lettres (bonus L2/L3) x multiplicateurs de mot (W2/W3) + bonus de longueur.</li>
                       ...
                       {/* Contenu principal mobile : classement + aperÃ§u mot + grille */}
<div className="flex-1 flex flex-col gap-2 px-3 pt-2 pb-3 overflow-hidden">
  {/* Classement centrÃ© sur le joueur â version bague + fenÃªtre */}
  <RankingWidgetMobile
  fullRanking={fullRanking}
  selfNick={selfNick}
  darkMode={darkMode}
...
/>
          ...
          {/* AperÃ§u du mot en cours / dernier message de statut */}
  ...
  <div className={`w-full text-center font-bold text-base flex items-center justify-center ${shake ? "shake" : ""}`}>
              {phase !== "playing" ? (
                ...
                <span className="text-slate-700 dark:text-slate-200">
                  {(currentDisplay || "PrÃªt Ã  jouer").toUpperCase()}
                </span>
              )}
            </div>
          </div>

                    {/* Grille sous l'aperÃ§u du mot, poussÃ©e vers le bas mais remontÃ©e visuellement */}
<div className="flex-1 flex justify-center">
  ...
  className={[
    ...
    // mobile : tuile = 100% de la cellule, carrÃ©e
        ...
        <div className="flex flex-wrap items-center gap-2 sm:gap-4 bg-white border rounded-xl px-3 py-2 shadow-sm">
          ...
          <div className="flex items-center gap-1.5 sm:gap-2">
            ...
            >
              ...
              <span className="sr-only">{isFullscreen ? "Quitter plein Ã©cran" : "Plein Ã©cran"}</span>
            </button>
            <button
              onClick={() => setShowHelp((v) => !v)}
              className="px-2 py-1 text-[11px] sm:px-3 sm:py-1.5 sm:text-xs font-semibold rounded-lg border bg-gray-100 hover:bg-gray-200 flex items-center justify-center"
...
{phase === "playing" && (
      ...
      <div className="flex-1 min-h-0">
        ...
        />
          ...
          {phase !== "playing" && finalRanking.length === 0 && (
            <div className="flex flex-col gap-2 flex-1 min-h-0">
              <div className="text-sm font-semibold">Joueurs connectÃ©s</div>
  ...
  type="button"
  className={[
    // plus de tailles figÃ©es en px ici
...
>
  ...
  {bonus && (
    ...
    <div className="flex justify-center items-center gap-1">
      {liveWord.split("").map((ch, idx) => {
        // rotation dÃ©terministe lÃ©gÃ¨re, entre -5Â° et +5Â°
        const angle = ((idx * 17 + liveWord.length * 13) % 11) - 5;
        return (
          <div
            key={idx}
