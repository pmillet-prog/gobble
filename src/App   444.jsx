...
function getVisibleViewportHeight() {
  ...
  const vv = window.visualViewport;
  const vvHeight =
    vv && Number.isFinite(vv.height) ? Math.round(vv.height) : 0;
  const winHeight = Number.isFinite(window.innerHeight)
    ? Math.round(window.innerHeight)
    : 0;
  return vvHeight > 0 ? vvHeight : winHeight;
}
...
export default function App() {
  ...
  useEffect(() => {
    ...
    const updateHeights = () => {
      const visibleH = readVisibleHeight();
      const chatActive = isChatOpenMobile || isChatClosing;
      const useVisualViewportForChat =
        vv && Number.isFinite(vv.height) && !chatActive;

      if (!chatActive) {
        if (visibleH > 0) {
          docEl.style.setProperty("--vvh", `${visibleH}px`);
        }
        chatBaselineHeightRef.current = 0;
        chatBodyLockHeightRef.current = 0;
        docEl.style.setProperty("--kb", "0px");
        setChatKeyboardInsetPx((prev) => (prev === 0 ? prev : 0));
        setChatViewportHeight((prev) => (prev === 0 ? prev : 0));
        return;
      }

      const nextBaseline = Math.max(chatBaselineHeightRef.current || 0, visibleH);
      if (nextBaseline > 0) {
        chatBaselineHeightRef.current = nextBaseline;
        chatBodyLockHeightRef.current = nextBaseline;
        setChatViewportHeight((prev) => (prev === nextBaseline ? prev : nextBaseline));
        if (!useVisualViewportForChat) {
          docEl.style.setProperty("--vvh", `${nextBaseline}px`);
        }
      }

      const delta = Math.max(0, nextBaseline - visibleH);
      const keyboardOpen = delta > KEYBOARD_INSET_THRESHOLD_PX;
      let kbInset = keyboardOpen ? delta : 0;
      if (nextBaseline > 0) {
        const maxInset = Math.round(nextBaseline * 0.55);
        kbInset = Math.min(kbInset, maxInset);
      }
      docEl.style.setProperty("--kb", `${kbInset}px`);
      setChatKeyboardInsetPx((prev) => (prev === kbInset ? prev : kbInset));
    };
 ...
 useEffect(() => {
  ...
  useEffect(() => {
    ...
    const applyLockedHeight = () => {
      const vv = window.visualViewport;
      // Prioritize visual viewport height to correctly size the body when the keyboard is open.
...
function playTileStepSound(step) {
  ...
  function captureChatViewportBaseline() {
    const baseHeight = getVisibleViewportHeight();
    if (baseHeight > 0) {
      const nextBase = Math.max(chatBodyLockHeightRef.current || 0, baseHeight);
      chatBodyLockHeightRef.current = nextBase;
      chatBaselineHeightRef.current = Math.max(
        chatBaselineHeightRef.current || 0,
        nextBase
      );
      setChatViewportHeight((prev) => (prev === nextBase ? prev : nextBase));
      if (typeof document !== "undefined") {
        document.documentElement.style.setProperty("--vvh", `${nextBase}px`);
      }
    }
  }
...
function handleTouchEnd() {
 ...
 showToast(`+${pts} pts`);
  ...
  if (showTournamentFinale) {
    ...
    const bc = typeof breakCountdown === "number" ? Math.max(0, breakCountdown) : null;
    const finaleChatOverlayStyle = undefined;
    const finaleChatSheetStyle = {
      height: `calc((var(--vvh, 100vh) - var(--kb, 0px)) * ${CHAT_SHEET_HEIGHT_RATIO})`,
      maxHeight: `calc((var(--vvh, 100vh) - var(--kb, 0px)) * ${CHAT_SHEET_HEIGHT_RATIO})`,
    };
  ...
  if (isMobileLayout && (phase === "playing" || phase === "results")) {
    ...
    const chatOverlayStyle = undefined;
