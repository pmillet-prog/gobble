...
export default function TournamentPodium({ players, self, preparedActors = null, roundLabel = "Mini-tournoi", sound = false, onOpenProfile, onComplete }) {
  ...
  React.useEffect(() => {
    if (preparedActors) { setActors(preparedActors); return; }
  ...
  }, [participants, attempt, preparedActors]);
  ...
  React.useEffect(() => { if (actors && complete) onComplete?.(); }, [actors, complete, onComplete]);
  ...
  return <section className="tournament-podium" aria-label="Célébration du mini-tournoi">
    ...
    <div className={`podium-performance ${actors ? "is-playing" : ""} ${complete ? "is-complete" : ""} ${reduced ? "is-reduced" : ""}`} key={`${attempt}-${!!actors}`}>
