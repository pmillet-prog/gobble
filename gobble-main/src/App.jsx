...
function playTileStepSound(step) {
  ...
  function fetchWeeklyStats(force = false) {
    ...
    fetch("/api/stats/weekly", { signal: controller.signal })
      .then((res) => {
  ...
  function shiftWeeklyBoard(delta) {
    ...
    if (!Number.isInteger(delta) || total === 0) return;
          ...
          >
            ...
            >
              ...
              <div className="relative px-4 pb-4">
                <button
                  type="button"
                  className="group absolute left-0 top-1/2 -translate-y-1/2 ml-1 rounded-full p-3 bg-white/10 text-white opacity-80 hover:opacity-100 transition transform hover:scale-105 active:scale-95"
                  onClick={() => shiftWeeklyBoard(-1)}
                  aria-label="Precedent"
                >
                  <span className="block w-3 h-3 border-t-2 border-l-2 border-current rotate-[-45deg]" />
                </button>
                ...
                <button
                  type="button"
                  className="group absolute right-0 top-1/2 -translate-y-1/2 mr-1 rounded-full p-3 bg-white/10 text-white opacity-80 hover:opacity-100 transition transform hover:scale-105 active:scale-95"
                  onClick={() => shiftWeeklyBoard(1)}
                  aria-label="Suivant"
                >
                  <span className="block w-3 h-3 border-t-2 border-r-2 border-current rotate-[45deg]" />
                </button>
              </div>
            </div>
          </div>,
